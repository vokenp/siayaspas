        ...
        <div class="widget-body">
           <div class="widget-main">
              ...
              <div class="form-group col-sm-6">
						<label class="col-sm-4 control-label " for="DepartmentCode"> Department Code </label>
						<div class="col-sm-8">
							<input type="text" id="DepartmentCode" name="DepartmentCode" placeholder="Enter Department Code" class="col-xs-12 col-sm-12" value="<?php echo $rst['DepartmentCode'];?>"  required="true" />
						</div>
					</div>
         ...
         <div class="row">
           <div class="form-group col-sm-6">
             <label class="col-sm-4 control-label " for="DepartmentName"> Department Name </label>
             <div class="col-sm-8">
               <input type="text" id="DepartmentName" name="DepartmentName" placeholder="Enter Department Name" class="col-xs-12 col-sm-12" value="<?php echo $rst['DepartmentName'];?>"  required="true" />
             </div>
           </div>
         </div>
         ...
         <div class="row">
           <div class="form-group col-sm-6">
             <label class="col-sm-4 control-label " for="DirectorateID"> Reports To</label>
             <div class="col-sm-8">
               <select name="DirectorateID" id="DirectorateID" placeholder="Enter DirectorateID" class="col-xs-11 col-sm-11 chosen-select" required="true">
                 <?php
                   $DirectorateID = $rst["DirectorateID"];
                   $where = " where 1=1 ";

                   if ($DirectorateID != "") {
                     $DirectorateName = $db->GetOne("select DirectorateName from tbl_directorates where S_ROWID='$DirectorateID'");
                     echo "<option value='$DirectorateID'>$DirectorateName</option>";
                     $where .= " and S_ROWID<>'$DirectorateID'";
                   }
                   else
                   {
                     echo "<option value=''></option>";
                   }
                   $getData = $db->Execute("select S_ROWID,DirectorateName from tbl_directorates $where");
                   while (!$getData->EOF) {
                     $DirectorateID = $getData->fields["S_ROWID"];
                     $DirectorateName = $getData->fields["DirectorateName"];
                     echo "<option value='$DirectorateID'>$DirectorateName</option>";
                     $getData->MoveNext();
                   }
                   ?>
