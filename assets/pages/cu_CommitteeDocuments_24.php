...
?>
            ...
            <?php echo $S_ROWID ; ?>
             <div>
            <label class="control-label " for="DocumentCategory"> Committee</label>
               <select name="DocumentCategory" id="DocumentCategory" class="col-xs-12 col-sm-12 chosen-select form-control">
                 <?php 
   
                 $DocCategory = $rst["DocumentCategory"];
                  list($CatName,$CategoryID) = explode('-',$rst["DocumentCategory"]);
                  $getCatName = $db->GetOne("select CommitteeName from assemblycommittees where S_ROWID='$CategoryID'");
                  $getList = $db->Execute("select *from assemblycommittees where S_ROWID<>'$CategoryID' ");
                  echo "<option value='$DocCategory'>$getCatName</option>";
                    while (!$getList->EOF) {
                      $DocCateName = $getList->fields["CommitteeName"];
           ...
           <div>
            <label class="control-label " for="DocumentDate"> Document Category</label>
             <select id="CommitteeDocType" name="CommitteeDocType"  class="col-xs-12 col-sm-12"  required="true" >
                <?php echo $rs->GetListItems($rst["CommitteeDocType"],"CommitteeDocType","edit");?>
              </select>
          </div><hr/>
