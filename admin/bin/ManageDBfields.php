...
$dict = NewDataDictionary($db);
 ...
 if (isset($_POST['CreateNewTbl']))
 {
	...
	$exec = $dict->executeSqlArray($sqlarray);
	 if ($exec) {
     $execu = $db->Execute("ALTER TABLE $TableName MODIFY DateCreated TIMESTAMP DEFAULT CURRENT_TIMESTAMP,MODIFY DateModified TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
     $execu = $db->Execute("insert into $TableName(CreatedBy) values('Admin')");
     $execu = $db->Execute("delete from $TableName");
	 	echo "Success";
	 }
 }
